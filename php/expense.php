...
if (($_GET['action'] ?? '') === 'recent') {
    $stmt = $pdo->prepare("SELECT e.*, c.name as category_name FROM expenses e JOIN categories c ON e.category_id = c.id WHERE e.user_id = ? ORDER BY e.date DESC, e.id DESC LIMIT 3");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
    ...
    $stmt = $pdo->prepare("UPDATE expenses SET amount = ?, category_id = ?, note = ?, date = ?, updated_at = NOW() WHERE id = ? AND user_id = ?");
